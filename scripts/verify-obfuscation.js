...
function calculateComplexity(filePath) {
    ...
    const metrics = {
        ...
        semicolonDensity: (content.match(/;/g) || []).length / (content.length / 1000),
...
async function testServerStartup(timeout = 10000) {
    return new Promise((resolve) => {
        ...
        child.stdout.on('data', (data) => {
            ...
            if (stdout.includes('listening') || stdout.includes('started') || stdout.includes('ready')) {
                child.kill();
        ...
        setTimeout(() => {
            child.kill();
            resolve({ success: true, note: 'Timeout (server likely waiting for DB)' });
        }, timeout);
    });
}
...
async function main() {
    console.log('\n' + '='.repeat(60));
    console.log(`${colors.bright}${colors.blue}  QvarryReact Obfuscation Verification${colors.reset}`);
    ...
    for (const filePath of files) {
        ...
        if (check.isObfuscated) {
            results.obfuscated++;
            if (verbose) log.success(`${relativePath} (${check.matchCount} patterns)`);
        } else {
    ...
    for (const filePath of files) {
        ...
        if (check.valid) {
            ...
            results.failures.push({ file: relativePath, reason: `Syntax error: ${check.error}` });
        }
    }
    console.log(`\n  Résultat: ${results.syntaxValid}/${results.total} fichiers valides\n`);

    // Test 3: Données sensibles
    console.log(`${colors.bright}Test 3: Absence de données sensibles${colors.reset}`);
    console.log('-'.repeat(40));
    ...
    for (const filePath of files) {
        ...
        if (check.clean) {
            ...
            if (verbose) {
                check.violations.forEach(v => console.log(`    Pattern: ${v.pattern}`));
            }
            results.warnings.push({ file: relativePath, violations: check.violations });
        }
    }
    console.log(`\n  Résultat: ${results.sensitiveDataFree}/${results.total} fichiers propres\n`);

    // Test 4: Fichiers critiques
    console.log(`${colors.bright}Test 4: Protection des fichiers critiques${colors.reset}`);
    console.log('-'.repeat(40));
    ...
    if (runServerTest) {
        ...
        if (serverTest.success) {
            log.success(`Serveur démarré avec succès ${serverTest.note ? `(${serverTest.note})` : ''}`);
        } else {
...
`);
    ...
    if (results.warnings.length > 0) {
        console.log(`  ${colors.yellow}⚠️  Avertissements: ${results.warnings.length}${colors.reset}`);
    }

    console.log('');

    if (allTestsPassed) {
        console.log(`  ${colors.green}${colors.bright}✅ TOUS LES TESTS PASSÉS${colors.reset}`);
        console.log(`  ${colors.green}Le code est correctement obfusqué et prêt pour la production.${colors.reset}`);
    } else {
        console.log(`  ${colors.red}${colors.bright}❌ CERTAINS TESTS ONT ÉCHOUÉ${colors.reset}`);
        console.log(`  ${colors.red}Veuillez corriger les problèmes avant le déploiement.${colors.reset}`);
    }
