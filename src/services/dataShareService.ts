 ...
 */
async function getUserDisplayName(userId: mongoose.Types.ObjectId): Promise<string> {
    try {
        const user = await UserModel.findById(userId).select('name surname pseudo showPseudo');
...
function getDataTypeLabel(dataType: DataType): string {
    switch (dataType) {
        case 'point': return 'un point';
        case 'fiche': return 'une fiche';
        case 'liste': return 'une liste';
        default: return 'des données';
    }
}
 ...
 */
async function getUserPublicKey(userId: mongoose.Types.ObjectId): Promise<string> {
    ...
    if (!publicKeyDoc) {
        throw new Error(`Clé publique RSA non trouvée pour l'utilisateur ${userId}`);
    }
 ...
 */
async function getUserPrivateKey(userId: mongoose.Types.ObjectId): Promise<string> {
 ...
 */
async function getFicheWithPoints(ficheId: mongoose.Types.ObjectId, userId: mongoose.Types.ObjectId) {
    ...
    const safeDecrypt = async (value: any): Promise<any> => {
        ...
        try {
            ...
            console.warn(`[SHARE] Erreur déchiffrement, valeur retournée telle quelle:`, e);
            return value;
        }
    };
 ...
 */
async function getListWithPoints(listId: mongoose.Types.ObjectId, userId: mongoose.Types.ObjectId) {
    ...
    const pointIdsFoundInMemory = new Set(pointsFromMemory.map((p: any) => p._id?.toString()));
 ...
 */
async function getPoint(pointId: mongoose.Types.ObjectId, userId: mongoose.Types.ObjectId) {
...
export async function shareData(
    ...
    // Vérification des contacts pour chaque destinataire
    ...
    for (const receiverId of receiverIds) {
        ...
        if (!contact) {
            throw new Error("Vous devez être contacts pour partager des données avec cet utilisateur.");
        }
    }
    ...
    switch (dataType) {
        case "fiche":
            ...
            relatedPointsIds = ficheData.points.map(p => p._id as mongoose.Types.ObjectId);
            break;

        case "liste":
            const listData = await getListWithPoints(dataId, senderId);
            dataToShare = listData;
            relatedPointsIds = listData.points.map(p => p._id as mongoose.Types.ObjectId);
            break;
    ...
    const dataShare = new DataShareModel({
        ...
        signature,
        dataHash,
        relatedPointsIds: relatedPointsIds.length > 0 ? relatedPointsIds : undefined,
        messagePerReceiver,
        sharedAt,
        expiresAt,
    ...
    for (const receiverId of receiverIds) {
        try {
            ...
            console.error(`❌ [DATA SHARE] Erreur notification pour ${receiverId}:`, notifError);
        }
    }
    ...
    console.log(`✅ [DATA SHARE] ${dataType} partagé par ${senderId} avec ${receiverIds.length} destinataire(s)`);
...
export async function getSharedData(
    ...
    const decryptedDataJSON = decryptWithPrivateKey(receiverData.encryptedData, receiverPrivateKey);

    // 4. Vérifier la signature avec la clé publique de l'émetteur
    const senderPublicKey = await getUserPublicKey(dataShare.senderId as mongoose.Types.ObjectId);
    const isSignatureValid = verifySignature(decryptedDataJSON, dataShare.signature, senderPublicKey);

    if (!isSignatureValid) {
        throw new Error("⚠️ Signature invalide ! Les données ont peut-être été altérées.");
    }
    ...
    if (dataShare.messagePerReceiver) {
        ...
        if (messageData) {
            decryptedMessage = decryptWithPrivateKey(messageData.encryptedMessage, receiverPrivateKey);
        }
    }
    ...
    console.log(`✅ [DATA SHARE] Données récupérées par ${receiverId} - Signature: ${isSignatureValid ? "VALIDE" : "INVALIDE"}`);
...
async function getSharedDataForCopy(
    ...
    console.log(`✅ [DATA SHARE COPY] Données récupérées pour copie - shareId: ${shareId}`);
...
export async function updateShareStatus(
    ...
    if (status === "accepted") {
        try {
            ...
            const receiverData = dataShare.encryptedDataPerReceiver[receiverDataIndex];
            const receiverPrivateKey = await getUserPrivateKey(receiverId);
            const decryptedDataJSON = decryptWithPrivateKey(receiverData.encryptedData, receiverPrivateKey);
            ...
            console.log(`✅ [DATA SHARE] Données préchargées pour copie - shareId: ${shareId}`);
        } catch (preloadError) {
            console.error(`❌ [DATA SHARE] Erreur lors du préchargement des données:`, preloadError);
            throw new Error(`Impossible de déchiffrer les données: ${(preloadError as Error).message}`);
        }
    }
    ...
    if (allResponded) {
        dataShare.isActive = false;
        await dataShare.save();
        console.log(`🗑️ [DATA SHARE] Partage ${shareId} désactivé - Tous les destinataires ont répondu`);
    }
    ...
    try {
        ...
        console.error(`❌ [DATA SHARE] Erreur notification pour l'expéditeur:`, notifError);
    }

    // Si accepté, copier les données dans le compte du destinataire
    if (status === "accepted" && preloadedData) {
        try {
            const copiedData = await copySharedDataToReceiver(receiverId, shareId, preloadedData);
            return { copiedData };
        } catch (error) {
            console.error(`❌ [DATA SHARE] Erreur lors de la copie des données:`, error);
            throw new Error(`Partage accepté mais erreur lors de la copie des données: ${(error as Error).message}`);
        }
    }
...
async function copySharedDataToReceiver(
    ...
    console.log(`📦 [DATA SHARE] Copie des données ${dataType} pour ${receiverId}... (session active: ${hasSession})`);

    switch (dataType) {
        case "point": {
            // Créer un nouveau point pour le destinataire
            const pointData = data.point || data;
            const locationDecrypted = normalizeLocation(data.location_decrypted || pointData.location);

            // Chiffrer TOUS les champs avec les clés du destinataire
            const locationEncrypted = await encryptUserKeys(receiverId, locationDecrypted);
            const nameEncrypted = await encryptUserKeys(receiverId, `${pointData.name} (partagé)`);
            const descriptionEncrypted = await encryptUserKeys(receiverId, pointData.description || "");
            const accessTypeEncrypted = pointData.accessType ? await encryptUserKeys(receiverId, pointData.accessType) : "";
            ...
            return { type: "point", id: newPoint._id.toString(), name: `${pointData.name} (partagé)` };
        }
        ...
        case "fiche": {
            ...
            // D'abord créer les points (avec chiffrement complet)
            const newPointIds: mongoose.Types.ObjectId[] = [];
            const newPointsForMemory: any[] = [];

            for (const pointData of pointsData) {
                const locationDecrypted = normalizeLocation(pointData.location_decrypted || pointData.location);

                // Chiffrer TOUS les champs avec les clés du destinataire
                const locationEncrypted = await encryptUserKeys(receiverId, locationDecrypted);
                const nameEncrypted = await encryptUserKeys(receiverId, pointData.name);
                const descriptionEncrypted = await encryptUserKeys(receiverId, pointData.description || "");
                const accessTypeEncrypted = pointData.accessType ? await encryptUserKeys(receiverId, pointData.accessType) : "";
                ...
                await newPoint.save();
                newPointIds.push(newPoint._id as mongoose.Types.ObjectId);

                // Préparer pour memoryStorage (données déchiffrées)
                const parsedLocation = parseLocation(locationDecrypted);
                newPointsForMemory.push({
                    _id: newPoint._id,
                    userId: receiverId,
                    name: pointData.name,
                    description: pointData.description || "",
            ...
            const ficheNameEncrypted = await encryptUserKeys(receiverId, `${ficheData.name} (partagé)`);
            const ficheVilleEncrypted = await encryptUserKeys(receiverId, toEncryptableString(ficheData.ville));
            const ficheTypeEncrypted = await encryptUserKeys(receiverId, toEncryptableString(ficheData.type));
            const ficheEtatEncrypted = await encryptUserKeys(receiverId, toEncryptableString(ficheData.etat));
            const ficheDifficulteEncrypted = ficheData.difficulte_acces ? await encryptUserKeys(receiverId, toEncryptableString(ficheData.difficulte_acces)) : undefined;
            const ficheRisqueEncrypted = ficheData.risque_oxygene ? await encryptUserKeys(receiverId, toEncryptableString(ficheData.risque_oxygene)) : undefined;
            const ficheAccesEncrypted = ficheData.acces_souterrain ? await encryptUserKeys(receiverId, toEncryptableString(ficheData.acces_souterrain)) : undefined;
            const fichePraticiteEncrypted = ficheData.praticite_souterrain ? await encryptUserKeys(receiverId, toEncryptableString(ficheData.praticite_souterrain)) : undefined;
            const ficheEtatGeneralEncrypted = ficheData.etat_general ? await encryptUserKeys(receiverId, toEncryptableString(ficheData.etat_general)) : undefined;
            const ficheEquipementEncrypted = ficheData.equipement_conseille ? await encryptUserKeys(receiverId, toEncryptableString(ficheData.equipement_conseille)) : undefined;
            const ficheSurfaceEncrypted = ficheData.surface ? await encryptUserKeys(receiverId, toEncryptableString(ficheData.surface)) : undefined;
            const ficheTypeGaleriesEncrypted = ficheData.type_galeries ? await encryptUserKeys(receiverId, toEncryptableString(ficheData.type_galeries)) : undefined;
            const ficheInteretsEncrypted = ficheData.interets ? await encryptUserKeys(receiverId, toEncryptableString(ficheData.interets)) : undefined;
            ...
            if (hasSession) {
                ...
                console.log(`✅ [DATA SHARE] Fiche et ${newPointIds.length} points ajoutés au memoryStorage`);
            }

            console.log(`✅ [DATA SHARE] Fiche copiée et chiffrée: ${newFiche._id} avec ${newPointIds.length} points`);
            return { type: "fiche", id: newFiche._id.toString(), name: `${ficheData.name} (partagé)` };
        }
        ...
        case "liste": {
            ...
            const listNameEncrypted = await encryptUserKeys(receiverId, `${listData.name} (partagé)`);
            const listDescriptionEncrypted = await encryptUserKeys(receiverId, listData.description || "");
            ...
            if (hasSession) {
                ...
                console.log(`✅ [DATA SHARE] Liste et ${newPointIds.length} points ajoutés au memoryStorage`);
            }

            console.log(`✅ [DATA SHARE] Liste copiée et chiffrée: ${newList._id} avec ${newPointIds.length} points`);
            return { type: "liste", id: newList._id.toString(), name: `${listData.name} (partagé)` };
        }
...
export async function cleanupExpiredShares(): Promise<number> {
    ...
    console.log(`🧹 [DATA SHARE CLEANUP] ${result.modifiedCount} partages expirés désactivés`);
