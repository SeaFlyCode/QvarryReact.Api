 ...
 */
function sanitizeSearchCriteria(criteria: Record<string, any>): Record<string, any> {
 ...
 */
export async function getAllFiches(): Promise<IFiche[]> {
 ...
 */
export async function getFicheById(ficheId: string): Promise<IFiche | null> {
    validateObjectId(ficheId, "ID de fiche");
    return await FicheModel.findById(ficheId);
}

/**
 * Récupère les fiches par utilisateur
 */
export async function getFichesByUserId(userId: string): Promise<IFiche[]> {
    validateObjectId(userId, "ID utilisateur");
    return await FicheModel.find({ userId: userId }).sort({ date_creation: -1 });
}

/**
 * Met à jour une fiche par son ID
 */
export async function updateFicheById(ficheId: string, updateData: Partial<IFiche>): Promise<IFiche | null> {
    validateObjectId(ficheId, "ID de fiche");
 ...
 */
export async function searchFiches(criteria: Record<string, any>): Promise<IFiche[]> {
 ...
 */
export async function findFichesNearLocation(longitude: number, latitude: number, maxDistance: number = 5000): Promise<IFiche[]> {
