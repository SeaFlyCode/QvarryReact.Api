...
class RefreshTokenService {
    private readonly REFRESH_TOKEN_EXPIRY = parseInt(process.env.REFRESH_TOKEN_EXPIRES_IN || '48') * 60 * 60 * 1000; // 48h par défaut
    ...
    private decryptIfPresent(value: string | undefined): string | undefined {
        ...
        try {
            ...
            console.warn('⚠️ [REFRESH_TOKEN] Valeur non chiffrée détectée, retour en clair');
            return value;
        }
    }
    ...
    async createRefreshToken(options: CreateRefreshTokenOptions): Promise<string> {
        ...
        const encryptedFingerprint = this.encryptIfPresent(options.deviceFingerprint);
        ...
        await RefreshTokenModel.create({
            ...
            tokenFamily: options.tokenFamily || crypto.randomBytes(16).toString('hex'),
            expiresAt,
            createdAt: new Date(),
            lastUsedAt: new Date(),
            revoked: false
     ...
     */
    async rotateToken(oldTokenId: string, newTokenId: string, ipAddress?: string, userAgent?: string): Promise<string> {
     ...
     */
    async detectTokenTheft(tokenFamily: string, userId: string, ipAddress?: string): Promise<boolean> {
        ...
        if (revokedTokenInFamily) {
            console.error(`🚨 [SECURITY] Vol de refresh token détecté! UserId: ${userId}, TokenFamily: ${tokenFamily}`);
    ...
    async cleanupExpiredTokens(): Promise<number> {
        const result = await RefreshTokenModel.deleteMany({
            $or: [
                ...
                { revoked: true, revokedAt: { $lt: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) } } // Révoqués depuis > 7 jours
            ]
        });
