...
export async function getAllPointsByUserId(userId: string): Promise<IPoint[]> {
    ...
    }).sort({ created_at: -1 }); // Trie par date décroissante
}

// Récupération d'un point par ID
export async function getPointById(pointId: string, userId: string): Promise<IPoint | null> {
...
export async function getPointsByFicheId(ficheId: string): Promise<IPoint[]> {
...
export async function linkPointToFiche(pointId: string, ficheId: string): Promise<IPoint | null> {
    if (!mongoose.Types.ObjectId.isValid(pointId) || !mongoose.Types.ObjectId.isValid(ficheId)) {
...
export async function unlinkPointFromFiche(pointId: string): Promise<IPoint | null> {
    if (!mongoose.Types.ObjectId.isValid(pointId)) {
        throw new Error("L'ID du point n'est pas valide.");
    }
