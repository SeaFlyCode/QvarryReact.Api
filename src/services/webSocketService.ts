...
import url from 'url';
import { sendMessage, getMessages, markMessageAsRead, replyToMessage, editMessage, deleteMessage } from '../controllers/messagesControllers';
...
const ALLOWED_WS_ORIGINS = NODE_ENV === 'production'
    ? [CLIENT_URL]
    : ['http://localhost:3000', 'http://localhost:3001', 'https://localhost:3000', 'https://localhost:3001', CLIENT_URL];
...
function canConnect(ip: string): { allowed: boolean; reason?: string } {
    ...
    if (attempt.count > MAX_CONNECTIONS_PER_MINUTE) {
        ...
        console.warn(`🚫 [WS RATE LIMIT] IP ${ip} bloquée pour ${BLOCK_DURATION_MS / 60000} minutes (${attempt.count} tentatives)`);
        return {
            allowed: false,
...
setInterval(() => {
    ...
    for (const [ip, attempt] of connectionAttempts.entries()) {
        if (attempt.firstAttempt < oneHourAgo && (!attempt.blockedUntil || attempt.blockedUntil < now)) {
...
class WebSocketService {
    ...
    private messageClients: Map<string, Map<string, Set<AuthenticatedWebSocket>>> = new Map(); // userId -> conversationId -> clients
    ...
    initialize(server: Server): void {
        ...
        console.log('🔌 WebSocket Server initialized on paths: /ws/notifications and /ws/messages');
        ...
        server.on('upgrade', (request, socket, head) => {
            ...
            const safeUrl = pathname + (parsedUrl.query.conv ? `?conv=${parsedUrl.query.conv}` : '');
            ...
            if (pathname === '/ws/notifications') {
                ...
                console.error(`❌ [WebSocket] Chemin inconnu: ${pathname}, connexion rejetée`);
                socket.destroy();
            }
        });

        this.notificationsWss.on('connection', this.handleNotificationConnection.bind(this));
        ...
        const notificationInterval = setInterval(() => {
            this.notificationsWss?.clients.forEach((ws: WebSocket) => {
                ...
                if (client.isAlive === false) {
                    console.log(`💔 [WS-Notifications] Client ${client.userId} non réactif, fermeture`);
        ...
        const messageInterval = setInterval(() => {
            this.messagesWss?.clients.forEach((ws: WebSocket) => {
                const client = ws as AuthenticatedWebSocket;
                if (client.isAlive === false) {
                    console.log(`💔 [WS-Messages] Client ${client.userId} non réactif, fermeture`);
                    return client.terminate();
                }
                client.isAlive = false;
                client.ping();
     ...
     */
    private async handleNotificationConnection(ws: WebSocket, request: any): Promise<void> {
        const client = ws as AuthenticatedWebSocket;
        client.isAlive = true;
        ...
        if (!rateLimitCheck.allowed) {
            console.warn(`⚠️ [WS-Notifications] Connexion refusée pour ${ip}: ${rateLimitCheck.reason}`);
        ...
        try {
            ...
            const decoded = jwt.verify(token, process.env.JWT_SECRET) as { id: string; type?: string; jti?: string };

            // WS-008: Vérifier que le token est bien de type 'websocket'
            if (decoded.type !== 'websocket') {
                console.error('❌ [WS-Notifications] Token invalide: type attendu "websocket"');
            ...
            if (decoded.jti) {
                const isTokenValid = await redisSessionService.consumeWsToken(decoded.jti);
                if (!isTokenValid) {
                    console.error(`❌ [WS-Notifications] Token déjà utilisé (jti: ${decoded.jti.substring(0, 8)}...)`);
                    client.close(4003, 'Token already used');
                    return;
                }
                console.log(`✅ [WS-Notifications] Token à usage unique validé (jti: ${decoded.jti.substring(0, 8)}...)`);
            }
            ...
            client.on('close', () => {
                if (client.userId) {
                    ...
                    console.log(`👋 [WS-Notifications] Client déconnecté : ${client.userId}`);
                }
            });
     ...
     */
    private async handleMessageConnection(ws: WebSocket, request: any): Promise<void> {
        const client = ws as AuthenticatedWebSocket;
        client.isAlive = true;
        ...
        if (!rateLimitCheck.allowed) {
            console.warn(`⚠️ [WS-Messages] Connexion refusée pour ${ip}: ${rateLimitCheck.reason}`);
            client.close(4029, rateLimitCheck.reason);
            return;
        }
        ...
        if (!token || !conversationId || !userId) {
            console.error('❌ [WS-Messages] Connexion refusée : paramètres manquants (token, conv, user)');
        ...
        try {
            ...
            // WS-008: Vérifier que le token est bien de type 'websocket'
            if (decoded.type !== 'websocket') {
                console.error('❌ [WS-Messages] Token invalide: type attendu "websocket"');
                client.close(4002, 'Invalid token type');
                return;
            }
            ...
            if (decoded.jti) {
                const isTokenValid = await redisSessionService.consumeWsToken(decoded.jti);
                if (!isTokenValid) {
                    console.error(`❌ [WS-Messages] Token déjà utilisé (jti: ${decoded.jti.substring(0, 8)}...)`);
                    client.close(4003, 'Token already used');
                    return;
                }
                console.log(`✅ [WS-Messages] Token à usage unique validé (jti: ${decoded.jti.substring(0, 8)}...)`);
            }
            ...
            if (!conversation) {
                console.error(`❌ [WS-Messages] Accès refusé: ${client.userId} n'est pas participant de la conversation ${conversationId}`);
            ...
            console.log(`✅ [WS-Messages] Client connecté : ${client.userId} pour conversation ${conversationId}`);
            ...
            client.on('message', async (message: Buffer) => {
                try {
                    // WS-003: Rate limiting par message
                    const now = Date.now();
                    if (now - (client.messageCountResetTime || 0) > MESSAGE_RATE_LIMIT_WINDOW_MS) {
                        client.messageCount = 0;
                    ...
                    if (client.messageCount > MAX_MESSAGES_PER_MINUTE) {
                        console.warn(`⚠️ [WS-Messages] Rate limit atteint pour ${client.userId} (${client.messageCount} msg/min)`);
                        client.send(JSON.stringify({
                            type: 'error',
                    ...
                    if (message.length > MAX_MESSAGE_SIZE) {
                        console.warn(`⚠️ [WS-Messages] Message trop volumineux de ${client.userId} (${message.length} bytes)`);
                        client.send(JSON.stringify({
                            type: 'error',
                    ...
                    const ALLOWED_MESSAGE_TYPES = ['message', 'getMessages', 'markMessageAsRead', 'replyToMessage', 'editMessage', 'deleteMessage'];
                    if (!data || typeof data !== 'object' || !data.type || !ALLOWED_MESSAGE_TYPES.includes(data.type)) {
                        console.warn(`⚠️ [WS-Messages] Type de message invalide de ${client.userId}: ${data?.type}`);
                        client.send(JSON.stringify({
                            type: 'error',
                    ...
                    if (data.type === 'message' && (!data.content || typeof data.content !== 'string' || data.content.length > 10000)) {
                        client.send(JSON.stringify({
                            type: 'error',
                    ...
                    if (!currentConversation) {
                        console.warn(`⚠️ [WS-Messages] Accès révoqué: ${client.userId} n'est plus participant de ${conversationId}`);
                        client.send(JSON.stringify({
                            type: 'error',
                            code: 'ACCESS_REVOKED',
                    ...
                    console.log(`📩 [WS-Messages] Message reçu de ${client.userId} dans conversation ${conversationId}:`, data.type);
                    ...
                    if (data.type === 'message') {
                            ...
                            status: (code: number) => ({
                                json: (obj: any) => {
                                    if (code === 201 && obj.messageId) {
                                        ...
                                        import('../models/messages').then(async ({ default: Message }) => {
                                            const fullMsg = await Message.findById(obj.messageId).lean();
                                            if (fullMsg && typeof fullMsg === 'object' && '_id' in fullMsg) {
                                                ...
                                                if (fullMsg.content) {
                                                    fullMsg.content = decryptCommunication(fullMsg.content);
                                                }
                                        ...
                                        client.send(JSON.stringify({ type: 'error', details: obj }));
                                    }
                                }
                            })
                        };
                        ...
                        const fakeRes: any = {
                            status: (code: number) => ({
                                json: (obj: any) => {
                                    client.send(JSON.stringify({ type: 'message_reply', ...obj }));
                            }
                        })
                        };
                        ...
                        const fakeRes: any = {
                            status: (code: number) => ({
                                json: (obj: any) => {
                                    client.send(JSON.stringify({ type: 'message_edited', ...obj }));
                            }
                        })
                        };
                        ...
                        const fakeRes: any = {
                            status: (code: number) => ({
                                json: (obj: any) => {
                                    client.send(JSON.stringify({ type: 'message_deleted', ...obj }));
                            }
                        })
                        };
                        await deleteMessage(fakeReq, fakeRes);
                    } else {
                        client.send(JSON.stringify({ type: 'error', details: 'Type de message non supporté' }));
                    }
                } catch (error) {
            ...
            client.on('close', () => {
                if (client.userId) {
                    ...
                    console.log(`👋 [WS-Messages] Client déconnecté : ${client.userId} de conversation ${conversationId}`);
                }
            });
            ...
            if (conversationClients) {
                conversationClients.forEach((client) => {
                    ...
                    if (client.readyState === WebSocket.OPEN && client.conversationId === conversationId) {
                        ...
                        console.log(`📤 [WS-Messages] Message diffusé à userId=${userId} (conversation=${conversationId})`);
                    }
                });
            }
        });
        console.log(`📊 [WS-Messages] Message diffusé à ${totalSent} client(s) pour conversation ${conversationId}`);
     ...
     */
    async notifyConversationUpdate(conversationId: string, message: any, excludeUserId?: string): Promise<void> {
        try {
            ...
            if (message.senderId) {
                try {
                    ...
                    const sender = await User.findById(message.senderId).select('name surname pseudo showPseudo').lean();
            ...
            console.log(`📊 [WS-Notifications] Mise à jour conversation ${conversationId} envoyée à ${participantIds.length} participant(s)`);
        } catch (error) {
            console.error('[WS-Notifications] Erreur lors de la notification de mise à jour:', error);
        }
    }
     ...
     */
    sendMessageToUserInConversation(userId: string, conversationId: string, message: any): void {
        ...
        if (!conversationClients || conversationClients.size === 0) {
            console.log(`⚠️ [WS-Messages] Utilisateur ${userId} non connecté à la conversation ${conversationId}`);
            return;
        }
        ...
        conversationClients.forEach((client) => {
            if (client.readyState === WebSocket.OPEN) {
                ...
                console.log(`📤 [WS-Messages] Message envoyé à ${userId} dans conversation ${conversationId}`);
            }
        });
    }
     ...
     */
    notifyMessagesRead(conversationId: string, readByUserId: string, messageIds: string[], participantUserIds: string[]): void {
        console.log(`👁️ [WS] Notifying messages read in ${conversationId} by ${readByUserId}`);
     ...
     */
    isUserConnectedToConversation(userId: string, conversationId: string): boolean {
        const userConversations = this.messageClients.get(userId);
        if (!userConversations) {
            console.log(`🔍 [WS] isUserConnectedToConversation(${userId}, ${conversationId}): false (pas de conversations)`);
            return false;
        }

        const conversationClients = userConversations.get(conversationId);
        if (!conversationClients || conversationClients.size === 0) {
            console.log(`🔍 [WS] isUserConnectedToConversation(${userId}, ${conversationId}): false (pas de clients pour cette conv)`);
            return false;
        }
        ...
        console.log(`🔍 [WS] isUserConnectedToConversation(${userId}, ${conversationId}): ${isConnected} (${openCount} clients ouverts sur ${conversationClients.size})`);
        return isConnected;
    }
     ...
     */
    notifyNewConversation(participantIds: string[], conversation: any, creatorId: string): void {
        console.log(`📬 [WS] Notification nouvelle conversation ${conversation._id} aux participants`);
        ...
        for (const userId of participantIds) {
            ...
            const userClients = this.clients.get(userId);
            if (!userClients || userClients.size === 0) {
                console.log(`⚠️ [WS] Utilisateur ${userId} non connecté pour new_conversation`);
                continue;
            }
     ...
     */
    notifyGroupDeleted(conversationId: string, participantIds: string[], deletedByUserId: string): void {
        console.log(`🗑️ [WS] Notification suppression groupe ${conversationId} aux participants`);
     ...
     */
    notifyGroupUpdate(conversationId: string, participantIds: string[], updateType: string, data: any): void {
        console.log(`🔄 [WS] Notification mise à jour groupe ${conversationId}: ${updateType}`);
    ...
    notifyMemberRemoved(conversationId: string, removedUserId: string): void {
        console.log(`👋 [WS] Notification membre retiré: ${removedUserId} du groupe ${conversationId}`);
            ...
            if (conversationClients) {
                conversationClients.forEach((client) => {
                    if (client.readyState === WebSocket.OPEN) {
                        ...
                        console.log(`🔒 [WS] Connexion messages fermée pour ${removedUserId} (conversation ${conversationId})`);
                    }
                });
                // Nettoyer la map
        ...
        if (!userClients || userClients.size === 0) {
            console.log(`⚠️ [WS] Utilisateur ${removedUserId} non connecté pour member_removed`);
            return;
        }
     ...
     * @param participantIds - IDs des participants à notifier
     */
    notifyGroupNameChanged(conversationId: string, newName: string, participantIds: string[]): void {
        console.log(`✏️ [WS] Notification changement de nom groupe ${conversationId}: "${newName}"`);
