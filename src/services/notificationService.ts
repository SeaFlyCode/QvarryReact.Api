...
export async function createNotification(
    ...
    const notification = new NotificationModel({
        ...
        read: false,
        createdAt: new Date(),
        relatedEntityId: data?.shareId?.toString() || data?.contactId?.toString() || data?.conversationId?.toString()
    });
...
export async function markAllNotificationsAsRead(
    ...
    console.log(`✅ [NOTIFICATION] ${result.modifiedCount} notifications marquées comme lues pour ${userId}`);
...
export async function cleanupOldNotifications(): Promise<number> {
    ...
    console.log(`🧹 [NOTIFICATION CLEANUP] ${result.deletedCount} anciennes notifications supprimées`);
...
export async function notifyShareReceived(
    ...
    console.log(`🔔 [SHARE] Notifications envoyées à ${receiverIds.length} destinataire(s)`);
}
