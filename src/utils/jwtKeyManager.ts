...
class JWTKeyManager {
    ...
    private loadKeys(): void {
        ...
        console.log(`🔑 [JWT KEY MANAGER] ${this.keys.size} clé(s) chargée(s), version active: v${this.currentVersion}`);
    }
    ...
    generateNewKey(): { key: string; version: number } {
        const nextVersion = Math.max(...Array.from(this.keys.keys()).map(Number)) + 1;
        const newKey = crypto.randomBytes(64).toString('base64');

        console.log(`🔑 [JWT KEY MANAGER] Nouvelle clé générée pour v${nextVersion}`);
        console.log(`   Ajoutez JWT_SECRET_V${nextVersion}=<VALEUR_RETOURNÉE> à vos variables d'environnement`);
        console.warn(`   ⚠️ SÉCURITÉ: La clé est retournée par cette fonction, ne la loguez JAMAIS`);
