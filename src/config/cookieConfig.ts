...
// COOKIE_SECURE: Si non défini, utilise true en production, false sinon
...
const COOKIE_SAMESITE: SameSiteOption = (process.env.COOKIE_SAMESITE as SameSiteOption) ||
...
export function getCookieConfig() {
    return {
        ...
        sameSite: COOKIE_SAMESITE,
        isProduction,
        jwtMaxAgeMinutes: parseInt(process.env.JWT_EXPIRES_IN?.replace(/[^0-9]/g, '') || '15'),
        refreshMaxAgeHours: parseInt(process.env.REFRESH_TOKEN_EXPIRES_IN || '48'),
    };
}

// Log de la configuration au démarrage
console.log(`🍪 [COOKIES] secure=${COOKIE_SECURE}, sameSite=${COOKIE_SAMESITE}`);
