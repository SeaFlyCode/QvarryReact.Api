...
import { createUser, deleteUserById, getAllUsers, getUserById, updateUserById, getUserByEmail } from "../services/userServices";
import bcrypt from "bcrypt";
import crypto from 'crypto';
import UserModel, { IUser, IUserBase } from "../models/users";
import { encrypt, decrypt } from "../utils/masterEncryptionUtils";
import { validatePasswordStrength, isPasswordInHistory, addToPasswordHistory } from "../utils/passwordUtils";
import { validateEmail } from "../utils/emailUtils";
import { sendWelcomeEmail, generateVerificationCode, sendVerificationEmail, sendAdminPendingValidationEmail, formatEmailDate } from "../services/emailService";
...
export async function handleCreateUser(req: Request, res: Response) {
    try {
        ...
        const ip_creation = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || 'unknown';
        ...
        if (existingUser) {
            return res.status(400).json({ 
                message: "Si cet email est associé à un compte, vous recevrez un email pour continuer." 
            });
        }
        ...
        console.log(`📧 [CREATE USER] Token de vérification généré: ${emailVerificationToken}`);
        console.log(`📧 [CREATE USER] Code de vérification: ${emailVerificationCode}`);
        ...
        sendWelcomeEmail(email, name, verificationLink, emailVerificationCode)
...
export async function handleUpdateUser(req: Request, res: Response) {
    try {
        const userId = req.params.id;
        const { name, surname, password, email, pseudo, showPseudo, currentPassword, login_notifications_enabled } = req.body;

        // Vérification obligatoire du mot de passe actuel
        if (!currentPassword) {
            return res.status(400).json({
                message: "Veuillez entrer votre mot de passe actuel pour modifier votre profil.",
        ...
        if (password) {
            ...
            const isInHistory = await isPasswordInHistory(password, allPasswordsToCheck);
            if (isInHistory) {
                console.warn(`⚠️ [UPDATE-USER] Tentative de réutilisation d'un ancien mot de passe pour userId: ${userId}`);
                return res.status(400).json({
        ...
        if (password) {
            ...
            console.log(`🔐 [UPDATE-USER] Mot de passe changé pour userId: ${userId}, ${revokedCount} tokens révoqués`);
        ...
        if (pseudo !== undefined) updatedUser.pseudo = pseudo ? encrypt(pseudo) : undefined;
        if (showPseudo !== undefined) updatedUser.showPseudo = showPseudo;
        if (login_notifications_enabled !== undefined) (updatedUser as any).login_notifications_enabled = login_notifications_enabled;
        ...
        await auditService.log({
            ...
            userAgent: req.headers['user-agent'],
            details: {
                fieldsUpdated: Object.keys(updatedUser).filter(k => k !== 'password' && k !== 'password_history')
            }
        });
...
export async function handleVerifyEmailByCode(req: Request, res: Response) {
    try {
        ...
        if (!targetUser) {
            return res.status(400).json({
                message: "Code invalide ou expiré. Veuillez vérifier le code ou en demander un nouveau.",
                expired: true
            });
        }
        ...
        const admins = await UserModel.find({ is_admin: true, is_blocked: false });
        ...
        for (const admin of admins) {
            ...
            sendAdminPendingValidationEmail(
                ...
                console.error(`❌ [EMAIL] Erreur envoi notification admin ${adminEmail}:`, err);
            });
        }

        console.log(`✅ [AUTH] Email vérifié pour ${decryptedUserEmail}. ${admins.length} admin(s) notifié(s).`);

        res.status(200).json({
            message: "Email vérifié avec succès ! Un administrateur doit maintenant valider votre compte. Vous recevrez un email lorsque votre compte sera activé.",
            verified: true,
 ...
 */
export async function handleResendVerificationEmail(req: Request, res: Response) {
    try {
        ...
        if (!user) {
            return res.status(200).json({
                message: "Si cet email est associé à un compte, un nouveau code de vérification sera envoyé."
            });
        }
        ...
        await sendVerificationEmail(email, userName, verificationLink, emailVerificationCode, '24 heures');
