...
import { sendWelcomeEmail, generateVerificationCode, sendPasswordResetEmail } from "../services/emailService";
...
export async function handleMobileLogin(req: Request, res: Response) {
    ...
    try {
        ...
        if (user.two_factor_enabled) {
            ...
            const { generateTwoFactorTempToken } = await import('./mobileTwoFactorControllers');
            const tempToken = generateTwoFactorTempToken(userId, mobileContext?.deviceId);
        ...
        console.log(`✅ [MOBILE-AUTH] Connexion réussie: ${email} (${mobileContext?.platform}) en ${loginDuration}ms`);
...
export async function handleMobileRegister(req: Request, res: Response) {
    ...
    try {
        ...
        if (existingUser) {
            ...
            return res.status(200).json({
                message: "Si cet email n'est pas déjà utilisé, un email de vérification sera envoyé.",
        ...
        console.log(`✅ [MOBILE-AUTH] Inscription réussie: ${email} (${mobileContext?.platform})`);
...
export async function handleMobileForgotPassword(req: Request, res: Response) {
    ...
    try {
        ...
        if (!user) {
            ...
            console.log(`[MOBILE-AUTH] Forgot password pour email inexistant: ${email}`);
        ...
        console.log(`📧 [MOBILE-AUTH] Email reset password envoyé: ${email}`);
...
export async function handleMobileRefreshToken(req: Request, res: Response) {
    ...
    try {
        ...
        if (tokenFamily) {
            ...
            if (isStolen) {
                console.error(`🚨 [MOBILE-SECURITY] Vol de token détecté pour userId: ${userId}`);
                return res.status(401).json({
        ...
        const { token: newAccessToken, tokenId: newTokenId } = generateSecureToken(userId, false);
