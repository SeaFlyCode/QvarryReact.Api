...
export async function sendMessage(req: Request, res: Response) {
    try {
        ...
        if (!conversation || !conversation.participants.some((p: any) => p.userId.toString() === userId)) {
        ...
        if (metadata && metadata.mentions) {
            encryptedMetadata = {
                ...metadata,
                mentions: metadata.mentions.map((id: string) => encryptCommunication(id))
            };
        } else if (metadata) {
        ...
        if (conversation.deletedBy && conversation.deletedBy.length > 0) {
            ...
            if (usersToReactivate.length > 0) {
                ...
                for (const reactivatedUserId of usersToReactivate) {
                    ...
                    console.log(`🔄 [MESSAGES] Réactivation conversation ${conversation._id} pour l'utilisateur ${userIdStr}`);
        ...
        for (const participant of otherParticipants) {
            ...
            await createNotification(
                ...
                'message',
                conversation.isGroup ? `Nouveau message dans ${conversationName}` : 'Nouveau message',
        ...
        res.status(500).json({ error: 'Erreur lors de l\'envoi du message', details: err });
    }
}
...
export async function getMessages(req: Request, res: Response) {
    try {
        ...
        const totalCount = await Message.countDocuments({ conversationId: new Types.ObjectId(conversationId) });
        ...
        const result = orderedMessages.map(msg => {
            ...
            return {
                ...
                replies: msg.replies?.map((r: IMessageReply) => ({ ...r, content: decryptCommunication(r.content) })) || [],
...
export async function markMessageAsRead(req: Request, res: Response) {
    try {
        ...
        const alreadyRead = message.readBy.map((id: string | Types.ObjectId) => id.toString()).includes(userId);
        ...
        res.status(500).json({ error: 'Erreur lors du marquage comme lu', details: err });
    }
}
...
export async function replyToMessage(req: Request, res: Response) {
    try {
        ...
        if (!messageId || !content) return res.status(400).json({ error: 'messageId et content requis' });
        const message = await Message.findById(messageId);
        if (!message) return res.status(404).json({ error: 'Message non trouvé' });
        // Chiffrer la réponse
        const encryptedContent = encryptCommunication(content);
...
export async function editMessage(req: Request, res: Response) {
    try {
        ...
        res.status(500).json({ error: 'Erreur lors de la modification', details: err });
    }
}
...
export async function markMessagesAsRead(req: Request, res: Response) {
    try {
        ...
        const notifResult = await NotificationModel.updateMany(
            {
                userId: userObjectId,
                type: 'message',
                messageId: { $in: messageIds.map((id: string) => new Types.ObjectId(id)) },
                read: false
            },
            {
                $set: {
        ...
        if (notifResult.modifiedCount > 0) {
            console.log(`✅ [MESSAGES] ${notifResult.modifiedCount} notification(s) marquée(s) comme lue(s)`);
        }
        ...
        if (result.modifiedCount > 0) {
            ...
            if (conversation) {
                ...
                console.log(`✅ [MESSAGES] ${result.modifiedCount} message(s) marqué(s) comme lu(s) par ${userId}`);
            }
        }
