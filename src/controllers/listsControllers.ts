...
export async function handleCreateList(req: Request, res: Response) {
    try {
        ...
        if (!userId || !name) {
            return res.status(400).json({ error: "L'identifiant utilisateur et le nom sont requis" });
        }
        ...
        if (!syncResult.success) {
            return res.status(500).json({ error: "Échec de la synchronisation avec la base de données" });
        }
...
export async function handleGetAllLists(req: Request, res: Response) {
    try {
        ...
        const lists = memoryStorage.getAllLists(userId);
...
export async function handleGetListById(req: Request, res: Response) {
    try {
        ...
        if (!userId || !listId) {
            return res.status(400).json({ error: "L'identifiant utilisateur et l'identifiant de la liste sont requis" });
        }
...
export async function handleUpdateList(req: Request, res: Response) {
    try {
        ...
        res.status(200).json({ message: "Liste mise à jour avec succès", updatedList });
    } catch (error: unknown) {
...
export async function handleGetListByUserId(req: Request, res: Response) {
    try {
        ...
        if (currentUserId !== targetUserId) {
            const user = await UserModel.findOne({ _id: currentUserId, is_admin: true });
            if (!user) {
                return res.status(403).json({
                    success: false,
                    message: "Accès refusé. Vous ne pouvez consulter que vos propres listes."
                });
            }
        }

        const lists = memoryStorage.getAllLists(targetUserId);

        res.status(200).json({
            success: true,
            message: `${lists.length} listes récupérées`,
            lists
        });
    } catch (error: unknown) {
        console.error(`Erreur lors de la récupération des listes pour l'utilisateur ${req.params.userId}:`, error);
        res.status(500).json({ 
            success: false,
...
export async function handleGetPointsByListId(req: Request, res: Response) {
    try {
        ...
        const points = memoryStorage.getPointsByListId(userId, listId);

        res.status(200).json({
            success: true,
            message: `${points.length} points récupérés`,
            points
        });
    } catch (error: unknown) {
        console.error(`Erreur lors de la récupération des points de la liste:`, error);
        res.status(500).json({
            success: false,
...
export async function handleGetListsByPointId(req: Request, res: Response) {
    try {
        ...
        const lists = memoryStorage.getListsByPointId(userId, pointId);

        res.status(200).json({
            success: true,
            message: `${lists.length} listes contiennent ce point`,
            lists
        });
    } catch (error: unknown) {
        console.error(`Erreur lors de la récupération des listes contenant le point:`, error);
        res.status(500).json({
            success: false,
...
export async function handleUpdateListPointsOrder(req: Request, res: Response) {
    try {
        ...
        const result = memoryStorage.updateListPointsOrder(userId, listId, pointIds);
        ...
        if (!syncResult.success) {
            return res.status(500).json({
                success: false,
                message: "L'ordre des points a été mis à jour en mémoire mais n'a pas pu être synchronisé avec la base de données",
        ...
        console.error(`Erreur lors de la mise à jour de l'ordre des points:`, error);
        res.status(500).json({
            success: false,
    ...
    try {
        ...
        for (const user of users) {
            try {
                ...
                console.error(`Erreur lors de la récupération des listes pour l'utilisateur ${user._id}:`, userError);
            }
        }
...
export async function handleBulkAddPointsToList(req: Request, res: Response) {
    try {
        ...
        if (!Array.isArray(pointIds) || pointIds.length === 0) {
            return res.status(400).json({
                success: false,
                message: "La liste des identifiants de points doit être un tableau non vide"
            });
        }
        ...
        console.error(`Erreur lors de l'ajout en batch de points à la liste:`, error);
        res.status(500).json({
            success: false,
 ...
 */
export async function handleBulkRemovePointsFromList(req: Request, res: Response) {
    try {
        const userId = req.user?.id;
        const { listId } = req.params;
        const { pointIds } = req.body;
        ...
        for (const pointId of pointIds) {
            const removed = memoryStorage.removePointFromList(userId, listId, pointId);
            if (removed) {
        ...
        console.error(`Erreur lors du retrait en batch de points de la liste:`, error);
        res.status(500).json({
            success: false,
