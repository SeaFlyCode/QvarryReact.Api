...
export const addContact = async (
  ...
  try {
    ...
    if (!contactCode || !contactCode.startsWith("@")) {
      res
        .status(400)
        .json({ error: "Code de contact invalide. Format attendu: @XXXXXX" });
      return;
    }
    ...
    const targetUser = await User.findOne({ contact_code: codeNumber })
      .select("_id contact_code")
      .lean();
    ...
    if (reverseContact) {
      await Contact.updateMany(
        {
          $or: [{ _id: newContact._id }, { _id: reverseContact._id }],
        },
        { status: "accepted" },
      );
      ...
      if (
        memoryStorage.hasSession((targetUser._id as Types.ObjectId).toString())
      ) {
        try {
          memoryStorage.storeContact(
      ...
      const targetUserFull = await User.findById(targetUser._id).select(
        "email name",
      );
          ...
          try {
            ...
            if (otherUser.showPseudo && decryptedPseudo) {
              contactInfo = {
                ...
                userCode: otherUser.contact_code
                  ? `@${otherUser.contact_code}`
                  : undefined,
              ...
              const decryptedName = otherUser.name
                ? decrypt(otherUser.name)
                : "Inconnu";
              const decryptedSurname = otherUser.surname
                ? decrypt(otherUser.surname)
                : "";
...
export const blockContact = async (
  ...
  try {
    ...
    if (typeof isBlocked !== "boolean") {
      res
        .status(400)
        .json({ error: "Le champ isBlocked doit être un booléen" });
      return;
    }
      ...
      try {
        ...
        if (contactUser.showPseudo && decryptedPseudo) {
          contactInfo = {
            ...
            surname: "",
            userCode: contactUser.contact_code
              ? `@${contactUser.contact_code}`
              : undefined,
            pseudo: decryptedPseudo,
            showPseudo: true,
          };
...
export const acceptContact = async (
  ...
  try {
    ...
    if (!contact) {
      res
        .status(404)
        .json({ error: "Demande de contact introuvable ou déjà traitée" });
      return;
    }
 ...
 */
export const refuseContact = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
