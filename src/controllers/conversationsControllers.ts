...
import { memoryStorage, MemoryStorageService } from '../services/memoryStorageService';
import { encrypt as encryptCommunication, decrypt as decryptCommunication } from '../utils/communicationEncryptionUtils';
...
export async function createPrivateConversation(req: Request, res: Response) {
    try {
        ...
        const contact = await Contact.findOne({
            $or: [
                { userId: userObjectId, contactId: participantObjectId, status: 'accepted', isBlocked: false },
                { userId: participantObjectId, contactId: userObjectId, status: 'accepted', isBlocked: false }
            ]
        });
        if (!contact) {
            return res.status(403).json({ error: 'Vous devez être contacts pour démarrer une conversation privée.' });
        }
        ...
        if (conversation) {
            ...
            if (wasDeletedByUser) {
                ...
                console.log(`🔄 [CONVERSATIONS] Réactivation de la conversation ${conversation._id} pour l'utilisateur ${userId}`);
                ...
                memoryStorage.storeConversation(userId, conversationToStore as IConversation);
        ...
        conversation = await Conversation.create({
            ...
            participants: [
                { userId: userObjectId, role: 'member', joinedAt: new Date(), leftAt: null },
                { userId: participantObjectId, role: 'member', joinedAt: new Date(), leftAt: null }
            ],
            lastMessage: null,
            createdAt: new Date(),
            updatedAt: new Date()
        ...
        memoryStorage.storeConversation(userId, conversationToStore as IConversation);
        memoryStorage.storeConversation(participantId, conversationToStore as IConversation);
        ...
        res.status(500).json({ error: 'Erreur lors de la création de la conversation privée', details: err });
    }
}
...
export async function createGroupConversation(req: Request, res: Response) {
    try {
        ...
        res.status(500).json({ error: 'Erreur lors de la création du groupe', details: err });
    }
}
    ...
    try {
        ...
        if (!conversations || conversations.length === 0) {
            ...
            dbConversations.forEach((conv: any) => {
                ...
                memoryStorage.storeConversation(userId!, conversationToStore as import('../models/conversations').IConversation);
            });
            conversations = dbConversations.map((conv: any) => ({
                _id: conv._id,
                name: conv.name ?? null,
                isGroup: conv.isGroup,
                creatorId: conv.creatorId ?? null,
        ...
        const result = await Promise.all(conversations.map(async (conv: import('../models/conversations').IConversation) => {
            ...
            // ═══════════════════════════════════════════════════════════════════════════
            // RÉCUPÉRATION DU DERNIER MESSAGE DIRECTEMENT DEPUIS LA BDD
            // ═══════════════════════════════════════════════════════════════════════════
            // On ne fait PAS confiance au cache pour le lastMessage car il peut être obsolète
            // On récupère toujours le dernier message de la conversation depuis la BDD
            try {
                ...
                if (lastMsg && 'content' in lastMsg && lastMsg.content) {
                    ...
                    const decryptedContent = await decryptCommunication(lastMsg.content as string);

                    // Format simplifié : "Vous : msg" si c'est moi, sinon juste "msg"
                    const senderId = (lastMsg as any).senderId?.toString();
                ...
                console.error(`[CONVERSATIONS] Erreur lors du chargement du dernier message:`, e);
            ...
            return {
                ...
                lastMessage: lastMessageContent, // ✅ Toujours à jour depuis la BDD
                unreadCount,
                createdAt: conv.createdAt,
                updatedAt: conv.updatedAt
            };
        ...
        res.status(500).json({ error: 'Erreur lors de la récupération des conversations', details: err });
    }
}
...
export async function getConversationDetails(req: Request, res: Response) {
    try {
        ...
        const isParticipant = conversation.participants.some((p: any) => p.userId.toString() === userId);
        if (!isParticipant) {
        ...
        res.json({
            ...
            name,
            isGroup: conversation.isGroup,
            participants: conversation.participants.map((p: any) => ({ userId: p.userId, role: p.role })),
            createdAt: conversation.createdAt
        });
    } catch (err) {
        res.status(500).json({ error: 'Erreur lors de la récupération des détails', details: err });
    }
}
...
export async function addGroupMembers(req: Request, res: Response) {
    try {
        ...
        if (!id || !userIds || !Array.isArray(userIds) || userIds.length === 0) {
            return res.status(400).json({ error: 'ID de conversation et userIds requis' });
        }
        ...
        const me = conversation.participants.find((p: any) => p.userId.toString() === userId);
        if (!me || me.role !== 'admin') {
        ...
        userIds.forEach((uid: string) => {
            if (!conversation.participants.some((p: any) => p.userId.toString() === uid)) {
        ...
        if (!added) return res.status(400).json({ error: 'Aucun nouveau membre à ajouter' });
        ...
        res.status(500).json({ error: 'Erreur lors de l\'ajout de membres', details: err });
    }
}
...
export async function removeGroupMember(req: Request, res: Response) {
    try {
        ...
        if (!id || !memberId) return res.status(400).json({ error: 'ID de conversation et userId requis' });
        const conversation = await Conversation.findById(id);
        if (!conversation || !conversation.isGroup) {
            return res.status(404).json({ error: 'Groupe non trouvé' });
        }
        ...
        if (memberId === userId && conversation.participants.filter((p: any) => p.role === 'admin').length === 1) {
            return res.status(400).json({ error: 'Impossible de retirer le dernier admin' });
        }
        const idx = conversation.participants.findIndex((p: any) => p.userId.toString() === memberId);
        ...
        console.log(`👋 [GROUPE] Membre ${memberId} retiré du groupe ${id} par ${userId}`);
...
export async function leaveGroup(req: Request, res: Response) {
    try {
        ...
        if (!userId) return res.status(401).json({ error: 'Utilisateur non authentifié' });
        const { id } = req.params;
        if (!id) return res.status(400).json({ error: 'ID de conversation requis' });
        const conversation = await Conversation.findById(id);
        if (!conversation || !conversation.isGroup) {
            return res.status(404).json({ error: 'Groupe non trouvé' });
        }
        const idx = conversation.participants.findIndex((p: any) => p.userId.toString() === userId);
        if (idx === -1) return res.status(404).json({ error: 'Vous n\'êtes pas membre de ce groupe' });
        // Ne pas laisser le dernier admin quitter
        if (conversation.participants[idx].role === 'admin' && conversation.participants.filter((p: any) => p.role === 'admin').length === 1) {
            return res.status(400).json({ error: 'Impossible de quitter, vous êtes le dernier admin' });
        }
        conversation.participants.splice(idx, 1);
        conversation.updatedAt = new Date();
        await conversation.save();
...
export async function deleteGroup(req: Request, res: Response) {
    try {
        ...
        const me = conversation.participants.find((p: any) => p.userId.toString() === userId);
        if (!me || me.role !== 'admin') {
            return res.status(403).json({ error: 'Seul un admin peut supprimer le groupe' });
        }
        ...
        const messagesToArchive = await Message.find({ conversationId: new Types.ObjectId(id) }).lean();
        ...
        const deletedMessages = await Message.deleteMany({ conversationId: new Types.ObjectId(id) });
        ...
        console.log(`🗑️ [GROUPE] Groupe ${id} supprimé par ${userId} (${deletedMessages.deletedCount} messages supprimés)`);
        ...
        res.status(500).json({ error: 'Erreur lors de la suppression du groupe', details: err });
    }
}
...
export async function updateGroupName(req: Request, res: Response) {
    try {
        ...
        const me = conversation.participants.find((p: any) => p.userId.toString() === userId);
        if (!me || me.role !== 'admin') {
            return res.status(403).json({ error: 'Seul un admin peut modifier le nom' });
        }
        ...
        console.log(`✏️ [GROUPE] Nom du groupe ${id} modifié en "${name}" par ${userId}`);
        res.json({ success: true, conversation: conversation.toObject() });
    } catch (err) {
        res.status(500).json({ error: 'Erreur lors du changement de nom', details: err });
    }
}
...
export async function updateGroupMemberRole(req: Request, res: Response) {
    try {
        ...
        if (!id || !memberId || !role) return res.status(400).json({ error: 'ID, userId et role requis' });
        const conversation = await Conversation.findById(id);
        if (!conversation || !conversation.isGroup) {
            return res.status(404).json({ error: 'Groupe non trouvé' });
        }
        ...
        const me = conversation.participants.find((p: any) => p.userId.toString() === userId);
        if (!me || me.role !== 'admin') {
            return res.status(403).json({ error: 'Seul un admin peut changer les rôles' });
        }
        const member = conversation.participants.find((p: any) => p.userId.toString() === memberId);
        ...
        res.status(500).json({ error: 'Erreur lors du changement de rôle', details: err });
    }
}
 ...
 */
async function updateMemoryForAllParticipants(participantIds: string[], conversation: IConversation, memoryStorage: MemoryStorageService) {
  for (const participantId of participantIds) {
    try {
      ...
      if (session) {
        ...
        console.log(`\u26A0\uFE0F [MemoryStorage] Pas de session en ligne pour l'utilisateur ${participantId}, la mémoire sera mise à jour à la prochaine connexion.`);
      }
    } catch (err) {
      // Log l'erreur mais ne bloque pas le processus
      console.error(`\u274C Erreur lors de la mise à jour mémoire pour l'utilisateur ${participantId}:`, err);
    }
  }
}
...
export async function markConversationAsRead(req: Request, res: Response) {
    try {
        ...
        const unreadMessages = await Message.find({
            ...
            readBy: { $ne: userObjectId }
        }).select('_id').lean() as { _id: Types.ObjectId }[];
        ...
        console.log(`✅ [CONVERSATIONS] ${result.modifiedCount} messages et ${notifResult.modifiedCount} notifications marqués comme lus pour l'utilisateur ${userId} dans la conversation ${id}`);
        ...
        if (unreadMessageIds.length > 0) {
            ...
            webSocketService.notifyMessagesRead(id, userId, unreadMessageIds, participantIds);
        }
    ...
    try {
        ...
        if (conversation.isGroup) {
            return res.status(400).json({
                error: 'Utilisez la fonction "quitter le groupe" pour les conversations de groupe',
                code: 'USE_LEAVE_GROUP'
            });
        }

        // Ajouter l'utilisateur à la liste deletedBy s'il n'y est pas déjà
        const alreadyDeleted = conversation.deletedBy?.some((id: Types.ObjectId) => id.toString() === userId);
        if (alreadyDeleted) {
        ...
        console.log(`✅ [CONVERSATIONS] Conversation ${id} masquée par l'utilisateur ${userId}`);
        ...
        res.json({
            success: true,
            message: 'Conversation masquée. Elle réapparaîtra si vous recevez un nouveau message.',
            hidden: true
        });
    } catch (err) {
...
export async function adminDeleteConversationPermanent(req: Request, res: Response) {
    try {
        let userId: string | undefined;
        let isAdmin = false;
        if (req.user && typeof req.user === 'object' && 'id' in req.user) {
        ...
        // Archiver tous les messages avant suppression (données chiffrées)
        const messagesToArchive = await Message.find({ conversationId: conversationId }).lean();
        for (const msg of messagesToArchive) {
            await dataArchiveService.archiveEntity(
                'message',
                msg._id as Types.ObjectId,
        ...
        console.log(`🗑️ [ADMIN] Conversation ${id} supprimée définitivement par admin ${userId} (${deletedMessages.deletedCount} messages supprimés)`);
