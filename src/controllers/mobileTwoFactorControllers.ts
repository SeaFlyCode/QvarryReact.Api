 ...
 */
function generateMobileToken(userId: string, isAdmin: boolean = false): { token: string; tokenId: string; keyVersion: string } {
 ...
 */
function generateMobileTokenWithDevice(userId: string, isAdmin: boolean = false, deviceId?: string): { token: string; tokenId: string; keyVersion: string } {
...
export async function mobileSetupTwoFactor(req: Request, res: Response): Promise<Response> {
    try {
...
export async function mobileVerifyAndEnableTwoFactor(req: Request, res: Response): Promise<Response> {
    try {
        const userId = req.user?.id;
        const { code } = req.body;
        const mobileContext = (req as any).mobileContext;
        ...
        if (!user.two_factor_secret) {
            return res.status(400).json({
                error: "Aucune configuration 2FA en attente. Initiez d'abord la configuration.",
        ...
        if (!isValid) {
            ...
            return res.status(400).json({
                error: "Code invalide. Vérifiez que l'heure de votre téléphone est correcte.",
                code: 'INVALID_CODE'
            });
        }
...
export async function mobileDisableTwoFactor(req: Request, res: Response): Promise<Response> {
    try {
        const userId = req.user?.id;
...
const twoFactorAttempts = new Map<string, { count: number; firstAttempt: Date; blockedUntil?: Date }>();

/**
 * Vérifie les tentatives 2FA pour un userId donné
 * Bloque après 5 échecs pendant 30 minutes
 */
async function checkTwoFactorAttempts(userId: string): Promise<{ allowed: boolean; waitTime?: number }> {
    ...
    if (attempt.blockedUntil && attempt.blockedUntil > now) {
        const waitMinutes = Math.ceil((attempt.blockedUntil.getTime() - now.getTime()) / 60000);
 ...
 */
export function generateTwoFactorTempToken(userId: string, deviceId?: string): string {
 ...
 */
function verifyTwoFactorTempToken(tempToken: string): { userId: string; deviceId?: string } | null {
    try {
        const { secret } = jwtKeyManager.getCurrentKey();
...
export async function mobileVerifyTwoFactorLogin(req: Request, res: Response): Promise<Response> {
    try {
        ...
        if (tempToken) {
            ...
            if (tokenDeviceId && mobileContext?.deviceId && tokenDeviceId !== mobileContext.deviceId) {
                console.warn(`🚨 [MOBILE-2FA] Device mismatch: token=${tokenDeviceId}, header=${mobileContext.deviceId}`);
                await auditService.log({
                    userId,
        ...
        if (isRecoveryCode) {
            ...
            if (recoveryAttempt && recoveryAttempt.count >= 3) {
                return res.status(429).json({
                    error: "Trop de tentatives avec les codes de récupération. Veuillez utiliser un code TOTP.",
            ...
            if (recoveryCodeIndex === -1) {
                ...
                if (recoveryAttempt) {
                    ...
                    twoFactorAttempts.set(recoveryKey, { count: 1, firstAttempt: new Date() });
                }
            ...
            console.log(`⚠️ [MOBILE-2FA] Code de récupération utilisé pour ${userId}, ${user.two_factor_recovery_codes!.length} restants`);
        } else {
            ...
            codeValid = speakeasy.totp.verify({
                ...
                encoding: 'base32',
                token: code,
                window: 0  // Un seul code valide (plus strict)
            });
        ...
        const { token, tokenId } = generateMobileTokenWithDevice(userId, user.is_admin || false, deviceId);
        ...
        console.log(`✅ [MOBILE-2FA] Login complété pour ${userId} (${mobileContext?.platform})`);

        const jwtMaxAge = parseInt(process.env.JWT_EXPIRES_IN?.replace(/[^0-9]/g, '') || '15') * 60;
        const refreshMaxAge = parseInt(process.env.REFRESH_TOKEN_EXPIRES_IN || '48') * 60 * 60;
...
export async function mobileRegenerateRecoveryCodes(req: Request, res: Response): Promise<Response> {
    try {
        const userId = req.user?.id;
        ...
        console.log(`🔄 [MOBILE-2FA] Codes de récupération régénérés pour ${userId}`);
...
export async function mobileGetTwoFactorStatus(req: Request, res: Response): Promise<Response> {
    try {
        const userId = req.user?.id;
