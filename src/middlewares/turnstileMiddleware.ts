  ...
  try {
    ...
    if (result.success) {
      ...
      return res.status(400).json({
        ...
        code: "CAPTCHA_FAILED",
        details:
          process.env.NODE_ENV === "development" ? errorCodes : undefined,
      });
    }
  } catch (error) {
 ...
 */
export const verifyTurnstileOptional = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  // Si pas de clé configurée, bypass en développement seulement
