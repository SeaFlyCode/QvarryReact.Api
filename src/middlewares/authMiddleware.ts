...
import { loadAndDecryptUserData } from "../controllers/authControllers";
...
export const authMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!token) {
            console.warn(`⚠️ [AUTH] Tentative d'accès sans token - ${req.method} ${req.path}`);
        ...
        // 3. VÉRIFICATION DE LA BLACKLIST (REDIS + FALLBACK)
        const isBlacklisted = await redisSessionService.isTokenBlacklisted(token) || blacklistedTokens.has(token);

        if (isBlacklisted) {
            console.warn(`🚫 [AUTH] Tentative d'utilisation d'un token révoqué - ${req.method} ${req.path}`);
            return res.status(401).json({
        ...
        if (keyVersion && jwtKeyManager.hasVersion(keyVersion)) {
            ...
            if (!versionedSecret) {
                console.error(`❌ [SECURITY] Clé JWT version ${keyVersion} non trouvée`);
                return res.status(401).json({
            ...
            if (!mainSecret) {
                console.error('❌ [SECURITY] JWT_SECRET non défini dans les variables d\'environnement');
        ...
        // MOBILE: Bypass memoryStorage, utilise sessionService (sessions créées au login mobile)
        // ─────────────────────────────────────────────────────────────────────
        const requireActiveSession = process.env.REQUIRE_ACTIVE_SESSION !== 'false';

        if (requireActiveSession) {
            if (isMobileToken) {
                // MOBILE: Vérifier via sessionService (pas memoryStorage)
                // Le login mobile crée une session via sessionService.createSession()
                ...
                const hasValidJti = decoded.jti ?
                    await redisSessionService.validateSessionJti(decoded.id, decoded.jti) : false;

                if (!hasValidJti) {
                    console.warn(`⚠️ [AUTH-MOBILE] Session/JTI invalide pour userId: ${decoded.id}`);
                    return res.status(401).json({
                ...
                if (!memoryStorage.hasSession(decoded.id)) {
                    console.log(`📱 [AUTH-MOBILE] Initialisation lazy de memoryStorage pour userId: ${decoded.id}`);
                    try {
                        await loadAndDecryptUserData(decoded.id);
                        console.log(`✅ [AUTH-MOBILE] Session memoryStorage initialisée pour userId: ${decoded.id}`);
                    } catch (initError) {
                        console.error(`❌ [AUTH-MOBILE] Erreur initialisation session:`, getErrorMessage(initError));
        ...
        if (decoded.jti && !isMobileToken) {
            ...
            if (!isValidJti) {
                console.warn(`🚫 [AUTH] JTI invalide ou token obsolète pour userId: ${decoded.id} - JTI: ${decoded.jti}`);
                return res.status(401).json({
                    message: "Token invalide ou session expirée. Veuillez vous reconnecter.",
                    code: 'JTI_INVALID',
                    tokenBlacklisted: true
                });
            }
        ...
        if (decoded.isAdmin) {
            const user = await UserModel.findById(decoded.id).select('is_admin is_blocked').lean();
            if (!user) {
            ...
            if (user.is_blocked) {
                console.warn(`🚫 [AUTH] Utilisateur bloqué tente d'accéder: ${decoded.id}`);
            ...
            if (decoded.isAdmin && !verifiedIsAdmin) {
                console.error(`🚨 [SECURITY] TENTATIVE D'ESCALADE DE PRIVILÈGES BLOQUÉE! userId: ${decoded.id} - IP: ${req.ip}`);
                ...
                try {
                    ...
                    console.error('❌ [AUDIT] Erreur lors du log de la tentative d\'escalade:', auditError);
                }

                // Blacklister le token pour empêcher toute réutilisation
                try {
                    await redisSessionService.blacklistToken(token, {
                        token,
                        userId: decoded.id,
                    ...
                    console.error('❌ [SECURITY] Erreur blacklist token compromis:', blacklistError);
                }
        ...
        if (process.env.LOG_AUTH_ACCESS === 'true') {
            ...
            console.log(`✅ [AUTH-${authType}] Accès autorisé - userId: ${decoded.id} - ${req.method} ${req.path}`);
        }
        ...
        const errorType = isErrorWithName(error, 'TokenExpiredError') ? 'expired' :
                          isErrorWithName(error, 'JsonWebTokenError') ? 'invalid' : 'error';
        console.error(`❌ [AUTH] Erreur d'authentification (${errorType}) - ${req.method} ${req.path}:`, getErrorMessage(error));
